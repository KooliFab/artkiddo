...
Every feature proposal and pull request must state its effect on the local
foundation, optional capabilities, public contracts, local schema/files, and
rollback. Public changes must keep the account-free path functional and
testable without network access. Do not add provider SDKs, backend identifiers,
wire formats, secrets, remote schemas, or monetization to this repository.
