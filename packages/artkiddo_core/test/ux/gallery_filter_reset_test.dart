...
void main() {
  ...
  test(
    'deleting the filtered child resets the filter to AllChildren and posts filterReset once (C9)',
    () async {
      final childrenRepo = container.read(childrenRepositoryProvider);

      final aResult = await childrenRepo.create(
        name: 'Léa',
        birthDate: DateTime(2021, 3, 14),
      );
      final bResult = await childrenRepo.create(
        name: 'Noah',
      ...
      final sub = container.listen(allChildrenStreamProvider, (previous, next) {
        next.whenData(
          (list) => events.add(list.map((c) => c.id.hashCode).toList()),
        );
      ...
      expect(
        events.last.length,
        2,
        reason: 'both children must be visible before any deletion',
      );
      ...
      expect(
        ...
        isNull,
        reason: 'no reset has happened yet',
      );
      ...
      expect(
        container.read(galleryFilterProvider),
        const AllChildren(),
        reason: 'filter must fall back to "Tous"',
      );
      expect(
        container.read(galleryFilterResetBannerProvider),
        'Léa',
      ...
      expect(
        container.read(galleryFilterResetBannerProvider),
        'Léa',
        reason: 'must not self-clear on repeated reads',
      );
