...
class Device {
  ...
  Device._(this.tempRoot, this.db, this.vault, this.children, this.masterpieces, this.engine, this.userId,
      this.cloudApi);
  ...
  static Future<Device> create({
    ...
    final tempRoot = await Directory.systemTemp.createTemp('artkiddo_sync_device_');
    final docsDir = Directory(p.join(tempRoot.path, 'docs'))..createSync(recursive: true);
    ...
    return Device._(tempRoot, db, vault, childrenRepo, masterpiecesRepo, engine, userId, cloudApi);
  }
...
Future<File> makeTestImage(Directory dir, String name, {int width = 40, int height = 30, int seed = 0}) async {
  final image = img.Image(width: width, height: height);
  img.fill(image, color: img.ColorRgb8((seed * 37) % 255, (seed * 61) % 255, (seed * 89) % 255));
...
Future<String> createSyncedMasterpiece(
  ...
  final source = await makeTestImage(device.tempRoot, 'src_$seed.jpg', seed: seed);
...
void main() {
  ...
  Future<Device> newDevice(String userId) async {
    final d = await Device.create(cloudApi: cloudApi, uploader: uploader, downloader: downloader, userId: userId);
    devices.add(d);
    return d;
  }
  ...
  group('foyer creation at first cloud send', () {
    test('creates a foyer and attaches this vault to it, as Parent', () async {
      ...
      expect(await a.vault.masterPiecesDirectory, isNotNull); // sanity: vault usable
    ...
    test('a second sync for the same account is idempotent — same foyer, no duplicate creation', () async {
      final a = await newDevice('user-a');
      await a.sync();
  ...
  group('D13 — join and restore are the same operation', () {
    test('a fresh device joining an already-populated foyer pulls everything (restore)', () async {
      final a = await newDevice('user-a');
      await a.sync(); // creates the foyer
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;

      final childId = (await a.children.create(name: 'Zoé', birthDate: DateTime(2019, 3, 1)) as ActionSuccess<String>)
          .value;
      for (var i = 0; i < 5; i++) {
        await createSyncedMasterpiece(a, childId: childId, story: 'oeuvre $i', seed: i);
      }
    ...
    test('restoring twice in a row creates no duplicates', () async {
      ...
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'Léo', birthDate: DateTime(2020, 1, 1)) as ActionSuccess<String>).value;
    ...
    test('a foyer with children but no masterpieces yet restores without error or a stuck screen', () async {
      // Annex §2.1: children pushed before any masterpiece existed, on the
      // old engine — must not crash or hang the new one.
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      await a.children.create(name: 'Bébé sans dessin', birthDate: DateTime(2025, 6, 1));
      await a.sync();
    ...
    test('40 local masterpieces + 5 already in the foyer converge to 45 on both sides', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'Foyer', birthDate: DateTime(2018, 1, 1)) as ActionSuccess<String>).value;
      for (var i = 0; i < 5; i++) {
      ...
      final b = await newDevice('user-b');
      final bChildId =
          (await b.children.create(name: 'Foyer', birthDate: DateTime(2018, 1, 1)) as ActionSuccess<String>).value;
      for (var i = 0; i < 40; i++) {
  ...
  group('D9 — delete wins over a concurrent modification', () {
    test('A deletes, then B pushes a stale content edit on the same row: the delete wins', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'C', birthDate: DateTime(2020, 1, 1)) as ActionSuccess<String>).value;
      final mpId = await createSyncedMasterpiece(a, childId: childId, story: 'original');
      await a.sync();
      ...
      // pushes afterwards.
      final editResult = await b.masterpieces.updateStory(id: mpId, story: 'édité par B, trop tard');
    ...
    test('B edits first (succeeds), then A deletes afterwards: still deleted, edit is moot', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
      ...
      await b.masterpieces.updateStory(id: mpId, story: 'édité par B, à temps');
      await b.sync();
    ...
    test('a deleted child cascades its tombstone to its masterpieces (D18), converging to both devices', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'À supprimer', birthDate: DateTime(2019, 1, 1)) as ActionSuccess<String>)
              .value;
  ...
  group('D9 — last-write-wins is by server arrival order, never the device clock', () {
    test('a device with its clock lied a year forward does not win just by claiming a later local time', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'C', birthDate: DateTime(2020, 1, 1)) as ActionSuccess<String>).value;
      final mpId = await createSyncedMasterpiece(a, childId: childId, story: 'v1');
      await a.sync();
      ...
      await a.masterpieces.updateStory(id: mpId, story: 'v2 depuis A (horloge trafiquée)');
  ...
  group('spec §5 — push isolation, backoff-free happy path for the rest', () {
    test('one masterpiece with a vanished local file does not block the other nine', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final childId =
          (await a.children.create(name: 'C', birthDate: DateTime(2020, 1, 1)) as ActionSuccess<String>).value;
  ...
  group('spec — a sync interrupted mid-drain resumes without duplicating or losing entries', () {
    test('one failing entry does not block the others, and retrying only the survivor never duplicates it',
        () async {
      final a = await newDevice('user-a');
      await a.sync();
      final childId =
      ...
      cloudApi.throwOnNextMasterpieceUpsert = Exception('simulated transient failure');
      ...
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      expect(cloudApi.masterpiecesInFoyer(foyerId), 5, reason: 'the failed entry never reached the server');
      ...
      await a.db.update(a.db.syncOutboxTable).write(const SyncOutboxTableCompanion(nextAttemptAt: Value(null)));
      ...
      expect(await a.engine.outbox.countPending(), 0);
      expect(cloudApi.masterpiecesInFoyer(foyerId), 6, reason: 'no duplicate rows created by the retried push');
    });

    test('a device restart (a fresh SyncEngine over the same local database) resumes a half-drained outbox',
        () async {
      final a = await newDevice('user-a');
      await a.sync();
      final childId =
      ...
      expect(summary.pushSucceeded, 3, reason: 'only the three never-attempted entries were pending');
      expect(cloudApi.masterpiecesInFoyer(foyerId), 6, reason: 'the first three were not re-sent');
      expect(await a.engine.outbox.countPending(), 0);
    });
  });

  group('C-06 §1 — vault isolation refuses to merge two foyers', () {
    test('signing into a different foyer than the one this vault is attached to throws, and touches nothing',
        () async {
      final a = await newDevice('user-a');
      ...
      final childId =
          (await a.children.create(name: 'Local', birthDate: DateTime(2021, 1, 1)) as ActionSuccess<String>).value;
      await a.sync();
      ...
      final bDevice = await newDevice('user-b'); // separate vault, only to mint a genuinely distinct foyer
      await bDevice.sync();
      ...
      expect(() => mismatchedEngine.syncAll(), throwsA(isA<FoyerMismatchException>()));
  ...
  group('D10 — thumbnails converge before the original', () {
    test('a device joining a populated foyer gets a thumbnail immediately; the display derivative stays deferred',
        () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      ...
      final downloadResult = await b.engine.ensureDisplayImageDownloaded(mpId);
    ...
    test('a display download failure is recorded, not thrown, and does not block the rest of the pull', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'C', birthDate: DateTime(2020, 1, 1)) as ActionSuccess<String>).value;
      final okId = await createSyncedMasterpiece(a, childId: childId, seed: 1);
      final failId = await createSyncedMasterpiece(a, childId: childId, seed: 2);
      await a.sync();
  ...
  group('C-12 — a purge converges even to a device that missed the soft-delete window entirely', () {
    test('device offline through soft-delete and purge alike still learns the row is gone', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
          (await a.children.create(name: 'Zoé', birthDate: DateTime(2019, 3, 1)) as ActionSuccess<String>).value;
      final mpId = await createSyncedMasterpiece(a, childId: childId);
      await a.sync();
    ...
    test('a device that DID see the soft-delete first is unaffected by the purge log (no double-processing)',
        () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      ...
      await a.masterpieces.delete(mpId);
      await a.sync();
      await b.sync(); // sees the tombstone the ordinary way, row gone locally already
      expect(await b.masterpieces.getById(mpId), isNull);

      cloudApi.purgeMasterpiece(mpId);
      await b.sync(); // purge-log entry for an id already absent locally: a no-op, not an error
  ...
  group('Audio stories sync & lazy download', () {
    test('push uploads audio, pull syncs metadata, and ensureAudioDownloaded fetches lazily', () async {
      final a = await newDevice('user-a');
      await a.sync();
      final foyerId = (await VaultMetaRepository(a.db).getFoyerId())!;
      final childId =
      ...
      expect(pulled.relativeAudioPath, isNull, reason: 'Audio is not downloaded eagerly on pull');
      ...
      final localAudio = await b.vault.resolveFile(afterDownload.relativeAudioPath!);
