...
void main() {
  testWidgets('ArtworkCard affiche l’icône micro quand hasAudio est true', (tester) async {
  ...
  testWidgets('ArtworkCard n’affiche pas l’icône micro quand hasAudio est false', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: SingleChildScrollView(
  ...
  test('ArtworkTile.hasAudio reflète fidèlement la présence d’un audio sur le chef-d’œuvre', () {
  ...
  testWidgets('Long press sur la tuile déclenche le zoom et démarre l’audio jusqu’au relâchement', (tester) async {
    final fakePlayer = FakeAudioPlayerService();
    final tempDir = Directory.systemTemp.createTempSync('artkiddo_peek_test_');
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          localVaultProvider.overrideWithValue(LocalVault(documentsDirProvider: () async => tempDir)),
        ],
        child: MaterialApp(
    ...
    final gesture = await tester.startGesture(tester.getCenter(find.byType(Hero).first));
