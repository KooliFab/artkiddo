...
void main() {
  ...
  setUp(() async {
    tempRoot = await Directory.systemTemp.createTemp('artkiddo_capture_regression_test_');
  ...
  test('save() stays retryable after a failure — "Réessayer" is not inert forever (D09)', () async {
    ...
    expect(retry, isA<ActionSuccess<String>>(), reason: 'a retry after a failed save must actually run, not be silently ignored');
  });

  test('Retour from a blank step-3 draft returns to step 2 with the photo intact (D03)', () async {
    final controller = container.read(captureControllerProvider(entry).notifier);
    ...
    expect(before.isDirty, isFalse, reason: 'no manual artist choice and an empty anecdote is the "blank draft" case');
    ...
    expect(after.step, CaptureStep.review, reason: 'must land on step 2, not the dead step-1 screen');
    expect(after.draft, isNotNull, reason: 'the draft must survive — the old retakePhoto() call destroyed it');
    expect(after.draft!.temporaryImagePath, draftPath);
    expect(await File(draftPath).exists(), isTrue, reason: 'the photo file itself must not be deleted');
  });

  test('a manually selected artist makes the step-3 draft dirty (D04 precondition for D03)', () async {
    final controller = container.read(captureControllerProvider(entry).notifier);
    ...
    expect(
      ...
      isTrue,
      reason: 'a manual artist choice must make the draft dirty, unlike an automatic preselection',
    );
  });
}
