...
void main() {
  ...
  test(
    'save() with a missing birth date blocks the write and reports the error',
    () async {
      final controller = container.read(
        childEditorControllerProvider(args).notifier,
      );
  ...
  test('a duplicate name is a warning only — save() still succeeds', () async {
    ...
    expect(
      sub.read().hasValue,
      isTrue,
      reason: 'watchAll() should have emitted by now',
    );
    ...
    expect(
      ...
      reason: 'a duplicate first name is a warning, not a blocking error',
    );
    final state = container.read(childEditorControllerProvider(args));
  ...
  test('typed name and birth date survive a failed submit attempt', () async {
    ...
    expect(
      state.name,
      'Noah',
      reason: 'a validation failure must never clear a typed field',
    );
  });
}
